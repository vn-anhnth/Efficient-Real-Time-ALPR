...
rank = int(os.environ.get('RANK', 0))  # torchrun provides RANK
...
class Trainer:
    ...
    def __init__(self, cfg, mode='train', task='rec'):
        ...
        from fastplateocr.rec.optimizer import build_optimizer
    ...
    def _init_rec_model(self):
        from fastplateocr.rec.losses import build_loss as build_rec_loss
        from fastplateocr.rec.metrics import build_metric as build_rec_metric
        from fastplateocr.rec.modeling import build_model as build_rec_model
        from fastplateocr.rec.postprocess import (
            build_post_process as build_rec_post_process,
        )
        ...
        if self.use_transformers:
            if self.cfg['Architecture']['algorithm'] == 'UniRec':
                from fastplateocr.rec.modeling.unirec_modeling.configuration_unirec import (
                    UniRecConfig,
                )
                from fastplateocr.rec.modeling.unirec_modeling.modeling_unirec import (
                ...
                from fastplateocr.rec.modeling.cmer_modeling.modeling_cmer import (
                    CMER,
                    CMERConfig,
                )
    ...
    def _init_det_model(self):
        from fastplateocr.det.losses import build_loss as build_det_loss
        from fastplateocr.det.metrics import build_metric as build_det_metric
        from fastplateocr.det.modeling import build_model as build_det_model
        from fastplateocr.det.postprocess import (
            build_post_process as build_det_post_process,
        )
    ...
    def train(self):
        ...
        if type(eval_batch_step) == list and len(eval_batch_step) >= 2:
            ...
            self.logger.info(
                f'During the training process, after the {start_eval_step}th iteration, '
                f'an evaluation is run every {eval_batch_step} iterations')
        ...
        best_str = f"best metric, {', '.join([f'{k}: {v}' for k, v in self.best_metric.items()])}"
    ...
    def eval_step(self, global_step, epoch):
        ...
        cur_metric_str = f"cur metric, {', '.join([f'{k}: {v}' for k, v in cur_metric.items()])}"
        ...
        if self.writer is not None:
            for k, v in cur_metric.items():
                if isinstance(v, (float, int)):
                    self.writer.add_scalar(f'EVAL/{k}', v,
                                           global_step)
